        ...
        <Select labelId={'address-label'} label={'Delivery Address'}
          ...
          {addressList.map((x: any, counter: number) => (
            <MenuItem value={x.id} key={counter}>
              <Stack direction={'row'}>
                <div style={{ height: '20px', color: 'black' }}>{imageForCategory(parseInt(x.category))}</div>
                <div style={{ paddingLeft: '10px', height: '20px', color: 'black' }}><Typography>{x.name}</Typography></div>
              </Stack>
            </MenuItem>
          ))}
        </Select>
